...
import type { NodeLibraryItem, NodeGraphNode } from "@/lib/nodegraph/types";
...
export function NodeInspectorPanel({
  ...
  const nodeAccent = node?.data.appearance?.borderColor ?? "#ff9d1c";
    ...
    <Card className="editor-panel flex h-full min-h-[32rem] flex-col overflow-hidden">
      <CardHeader className="space-y-3 border-b border-white/6 pb-5">
        ...
        <CardDescription className="leading-6 text-muted-foreground">
          Adjust graph metadata and selected node details without leaving the canvas workflow.
        </CardDescription>
      </CardHeader>
          ...
          <TabsContent value="graph" className="mt-4 flex-1">
            <div className="space-y-4">
              ...
              <div className="rounded-[1.25rem] border border-white/8 bg-black/20 p-4 text-sm leading-7 text-muted-foreground">
                Use the graph tab for session-level naming and summary text. Node-level labels and
                field values stay in the adjacent node tab.
              </div>
            </div>
          </TabsContent>

          <TabsContent value="node" className="mt-4 min-h-0 flex-1">
            {node ? (
              ...
              <div className="rounded-[1.25rem] border border-dashed border-white/10 bg-black/20 p-5 text-sm leading-7 text-muted-foreground">
                Select a node on the canvas to edit its label, description, and custom field values.
              </div>
            )}
          </TabsContent>
        </Tabs>
