using System;
using System.Collections.Generic;
using System.Runtime.Serialization;

namespace CentralService.Service.Models
{
    /// <summary>
    /// 表示中心服务中登记的服务实例信息。
    /// </summary>
    [DataContract]
    public sealed class ServiceInfo
    {
        /// <summary>
        /// 获取或设置服务实例标识。
        /// </summary>
        [DataMember(Name = "id", Order = 1)]
        public string Id { get; set; }

        /// <summary>
        /// 获取或设置服务名称。
        /// </summary>
        [DataMember(Name = "name", Order = 2)]
        public string Name { get; set; }

        /// <summary>
        /// 获取或设置服务主机地址。
        /// </summary>
        [DataMember(Name = "host", Order = 3)]
        public string Host { get; set; }

        /// <summary>
        /// 获取或设置服务实例的局域网IP。
        /// </summary>
        [DataMember(Name = "localIp", Order = 4, EmitDefaultValue = false)]
        public string LocalIp { get; set; } = string.Empty;

        /// <summary>
        /// 获取或设置服务实例的运营商IP。
        /// </summary>
        [DataMember(Name = "operatorIp", Order = 5, EmitDefaultValue = false)]
        public string OperatorIp { get; set; } = string.Empty;

        /// <summary>
        /// 获取或设置服务实例的公网IP。
        /// </summary>
        [DataMember(Name = "publicIp", Order = 6, EmitDefaultValue = false)]
        public string PublicIp { get; set; } = string.Empty;

        /// <summary>
        /// 获取或设置服务端口。
        /// </summary>
        [DataMember(Name = "port", Order = 7)]
        public int Port { get; set; }

        /// <summary>
        /// 获取或设置服务完整访问地址。
        /// </summary>
        [DataMember(Name = "url", Order = 8, EmitDefaultValue = false)]
        public string Url { get; set; }

        /// <summary>
        /// 获取或设置服务类型。
        /// </summary>
        [DataMember(Name = "serviceType", Order = 9, EmitDefaultValue = false)]
        public string ServiceType { get; set; }

        /// <summary>
        /// 获取或设置服务状态值。
        /// </summary>
        [DataMember(Name = "status", Order = 10, EmitDefaultValue = false)]
        public int Status { get; set; }

        /// <summary>
        /// 获取或设置中心服务通过 WebSocket 向该服务发送心跳请求的频率（秒）。
        /// 为 0 时表示不发送心跳请求。
        /// </summary>
        [DataMember(Name = "heartbeatIntervalSeconds", Order = 11, EmitDefaultValue = false)]
        public int HeartbeatIntervalSeconds { get; set; }

        /// <summary>
        /// 获取或设置注册时间。
        /// </summary>
        [DataMember(Name = "registerTime", Order = 12, EmitDefaultValue = false)]
        public string RegisterTime { get; set; }

        /// <summary>
        /// 获取或设置最近一次心跳时间。
        /// </summary>
        [DataMember(Name = "lastHeartbeatTime", Order = 13, EmitDefaultValue = false)]
        public string LastHeartbeatTime { get; set; }

        /// <summary>
        /// 获取或设置负载均衡权重。
        /// </summary>
        [DataMember(Name = "weight", Order = 14, EmitDefaultValue = false)]
        public int Weight { get; set; }

        /// <summary>
        /// 获取或设置服务元数据。
        /// </summary>
        [DataMember(Name = "metadata", Order = 15, EmitDefaultValue = false)]
        public Dictionary<string, string> Metadata { get; set; }

        /// <summary>
        /// 获取或设置当前服务是否位于本地网络。
        /// </summary>
        [DataMember(Name = "isLocalNetwork", Order = 16, EmitDefaultValue = false)]
        public bool IsLocalNetwork { get; set; }
    }
}
