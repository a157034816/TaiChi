...
- 编辑器支持从“当前草稿图”直接打开独立可视化调试页：`/editor/{sessionId}/debug`。
- 可视化调试页会把当前草稿图提交到宿主侧调试会话接口，并支持在页面中动态设置/取消节点级断点。
...
- `POST /api/editor/sessions/{sessionId}/debug`：基于当前草稿图创建或刷新宿主调试会话
- `GET /api/editor/sessions/{sessionId}/debug`：读取当前宿主调试会话快照
- `POST /api/editor/sessions/{sessionId}/debug/step`：单步推进当前调试会话
- `POST /api/editor/sessions/{sessionId}/debug/continue`：继续运行当前调试会话
- `PUT /api/editor/sessions/{sessionId}/debug/breakpoints`：动态替换当前调试会话的断点列表
...
- 可视化调试页中的画布为只读视图，但每个节点都支持直接设断点/取消断点
- 调试页优先复用已有宿主调试会话；如果不存在活动会话，会自动用当前草稿图重新创建
