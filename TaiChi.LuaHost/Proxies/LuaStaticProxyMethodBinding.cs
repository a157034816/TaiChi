...
/// <summary>
/// 负责将 Lua 侧方法调用绑定到目标类型的静态方法。
/// </summary>
internal sealed class LuaStaticProxyMethodBinding
{
    private readonly LuaState _state;
    private readonly Type _targetType;
    private readonly LuaTable _selfTable;
    ...
    private readonly Dictionary<string, LuaFunction> _functionCache = new(StringComparer.Ordinal);
    ...
    /// <param name="selfTable">对应的静态代理壳表（用于判断 ':' 调用）。</param>
    public LuaStaticProxyMethodBinding(LuaState state, Type targetType, LuaTable selfTable)
    {
        _state = state ?? throw new ArgumentNullException(nameof(state));
        _targetType = targetType ?? throw new ArgumentNullException(nameof(targetType));
        _selfTable = selfTable ?? throw new ArgumentNullException(nameof(selfTable));
    ...
    public bool TryGetFunction(string memberName, out LuaFunction function)
    {
        ...
        function = CreateFunction(memberName, resolved);
        lock (_functionCache)
        {
            _functionCache[memberName] = function;
        }
    ...
    private LuaFunction CreateFunction(string memberName, MethodInfo[] methods)
    {
        if (methods.Length == 1)
        {
            return CreateBoundStaticMethodFunction(methods[0], memberName);
        }
        ...
        return new LuaFunction((_, _) => throw new LuaMappingException($"方法 {memberName} 存在多个重载，Lua 静态类型代理不支持自动选择重载，请为每个重载标注 [LuaOverloadPreferred(\"...\")] 并通过别名调用。"));
    }

    private LuaFunction CreateBoundStaticMethodFunction(MethodInfo method, string luaName)
    {
        return new LuaFunction(async (context, token) =>
        {
            try
            {
                return await InvokeStaticMethodAsync(context, token, method, luaName).ConfigureAwait(false);
            }
            ...
            {
                throw new LuaMappingException($"调用静态方法 {_targetType.Name}.{method.Name} 失败。", ex.InnerException);
            }
            catch (Exception ex)
            {
                throw new LuaMappingException($"调用静态方法 {_targetType.Name}.{method.Name} 失败。", ex);
            }
        });
    }

    private async ValueTask<int> InvokeStaticMethodAsync(LuaFunctionExecutionContext context, CancellationToken token, MethodInfo method, string luaName)
    {
        EnsureNotColonCall(context, luaName);

        var args = BindArguments(method, context, token);
        var result = method.Invoke(obj: null, args);
        return await CompleteInvocationAsync(context, result).ConfigureAwait(false);
    }

    private void EnsureNotColonCall(LuaFunctionExecutionContext context, string luaName)
    {
        ...
        try
        {
            candidate = context.GetArgument<LuaTable>(0);
        }
        catch
        {
            return;
        ...
        if (ReferenceEquals(candidate, _selfTable))
        {
            throw new LuaMappingException($"静态方法 {luaName} 不支持使用 ':' 调用（':' 会隐式传入 self），请使用 '.'（例如 static.{_targetType.Name}.{luaName}(...)）。");
        }
    }
    ...
    private static IReadOnlyDictionary<string, MethodInfo[]> BuildMethodMapByName(Type type)
    {
        ...
        foreach (var method in EnumerateEligibleStaticMethods(type))
        {
    ...
    private static IEnumerable<MethodInfo> EnumerateEligibleStaticMethods(Type type)
    {
